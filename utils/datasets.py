...
def min_rect(label):
    # label = torch.tensor(label,dtype=torch.float64).value
    x = label[:, :, 1::2]
    y = label[:, :, 2::2]
    cls = label[:, :, :1]
   ...
   # print(cls.shape)
    ...
    #print(w,h)
    #print(x_min.shape)
    rect_label = np.concatenate((cls,
                                 x_max - w / 2,
                                 y_max - h / 2,
                                 w,
                                 h),axis=1)
    print(rect_label.shape)
    return rect_label
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        rect_labels = (min_rect(np.array(labels, dtype=np.float64)),)
        self.labels = list(labels)
        self.rect_labels = list(rect_labels)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                if random.random() < 0.8:
                    img2, labels2 = load_mosaic(self, random.randint(0, len(self.rect_labels) - 1))
                else:
                    img2, labels2 = load_mosaic9(self, random.randint(0, len(self.rect_labels) - 1))
            ...
            labels = self.rect_labels[index].copy()
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # Labels
        labels, segments = self.rect_labels[index].copy(), self.segments[index].copy()
        if labels.size:
