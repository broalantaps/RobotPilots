    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                #labels[:, 1:] = xyxyxyxy2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
...
def load_samples(self, index):
    ...
    labels4 = np.concatenate(labels4, 0)
    for x in (labels4[:, 1:], *segments4):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()    #it may need edit! broalantaps
    # img4, labels4 = replicate(img4, labels4)  # replicate
...
def pastein(image, labels, sample_labels, sample_images, sample_masks):
    ...
    for s in scales:
        ...
        box = np.array([xmin, ymin, xmax, ymax], dtype=np.float32)
        if len(labels):
            ioa = bbohx_ioa(box, labels[:, 1:5])  # intersection over area
        else:
            ioa = np.zeros(1)
